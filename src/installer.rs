...
impl Installer {
    ...
    pub fn install(&self) {
        ...
        Artisan::check_installation(); // <= It closes the process if an installation is found
        
        match self.write_dotenv() {
            Ok(_) => {
                Artisan::cmd("Caching config...", "php", vec!["artisan", "config:cache"]);
                Artisan::migrate();
                Artisan::seed();
                ...
                console::print(&bold("\nMicelab is configured. Check your .env file for additional custom configuration"));
